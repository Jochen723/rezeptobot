$(document).ready(function(){
    ...
    function onEntry() {
        ...
        $.ajax({
            type:'GET',
            url:'../backend/getRezeptdetails.php',
            dataType: "json",
            data: {
    ...
    function onRezeptaenderungSpeichern() {
        if (document.getElementById("rezepttitel").value == "") {
            ...
            $.ajax({
                type:'POST',
                url:'../backend/postGeaendertesRezept.php',
                data: {event: JSON.stringify(rezept)},
                dataType: "json",
                success:function(data){
                    window.location.href = "../../rezeptdetails/masken/rezeptdetails.php?q=" + getUrlVariables()["q"];
                },
                error: function (xhr, txtStatus, errThrown) {
                    console.log(
                        "XMLHttpRequest: ", xhr,
